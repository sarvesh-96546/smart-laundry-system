...
export default function OrderDetails() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#080808] flex items-center justify-center">
      <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#8ff5ff]"></div>
    </div>
  );
  ...
  if (error) return (
    <div className="min-h-screen bg-[#080808] flex flex-col items-center justify-center p-4">
      <div className="bg-red-500/10 border border-red-500/20 p-8 rounded-3xl text-center max-w-md">
        ...
        <Link to="/admin" className="inline-block px-6 py-3 bg-white text-black font-bold rounded-xl hover:bg-[#8ff5ff] transition-colors">
          Return to Hub
        </Link>
      </div>
    </div>
  ...
  if (!orderData || !orderData.order) return (
    <div className="min-h-screen bg-[#080808] text-white flex items-center justify-center">
      <div className="text-center">
        ...
        <Link to="/admin" className="px-6 py-3 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-[#8ff5ff] hover:text-black transition-all">Return to Hub</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white font-['Plus_Jakarta_Sans'] pb-20 animate-in fade-in duration-1000">
      <div className="max-w-4xl mx-auto pt-24 px-6 md:px-0">
        <div className="flex justify-between items-center mb-8">
          <Link to={user ? "/admin" : "/"} className="text-slate-500 hover:text-[#8ff5ff] flex items-center gap-2 group transition-colors text-sm font-bold">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="relative overflow-hidden rounded-[2rem] bg-[#111] border border-white/5 p-8 shadow-2xl">
              <div className="absolute top-0 right-0 w-64 h-64 bg-[#8ff5ff]/5 blur-[100px] -mr-32 -mt-32"></div>
              
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-10">
                  <div>
                    <span className="text-[10px] font-bold text-[#8ff5ff] uppercase tracking-[0.2em] mb-2 block">Protocol Identification</span>
                  ...
                  <div className="text-right">
                    <span className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest ${
                      order.status === 'Delivered' ? 'bg-green-500/10 text-green-400' : 'bg-[#8ff5ff]/10 text-[#8ff5ff]'
                    }`}>
                      {order.status}
                    </span>
                  </div>
                ...
                <div className="mb-12">
                      ...
                      {STAGES.map((s, idx) => (
                        <div key={s} className="flex flex-col items-center gap-2 group relative">
                           <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold border transition-all duration-500 ${
                             idx <= currentStageIndex ? 'bg-[#8ff5ff] border-[#8ff5ff] text-black shadow-[0_0_20px_rgba(143,245,255,0.3)]' : 'bg-[#1a1a1a] border-white/10 text-slate-600'
                           }`}>
                             {idx + 1}
                           </div>
                   ...
                   <div className="relative h-1.5 bg-white/5 rounded-full overflow-hidden">
                      <div 
                        className="absolute h-full bg-[#8ff5ff] transition-all duration-1000 shadow-[0_0_15px_#8ff5ff]"
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-sm text-[#8ff5ff]">₹{order.amount}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-bold text-sm text-[#8ff5ff]">
            ...
            <div className="bg-[#111] border border-white/5 p-8 rounded-[2rem]">
              <h2 className="text-sm font-bold mb-8 flex items-center gap-2">
                <span className="material-symbols-outlined text-[#8ff5ff] text-base">history</span>
                Event Logs
              </h2>
                ...
                {updates.map((update, idx) => (
                  <div key={idx} className="relative pl-12 animate-in slide-in-from-left duration-500">
                    <div className={`absolute left-0 top-1 w-9 h-9 rounded-full flex items-center justify-center z-10 border ${
                      idx === 0 ? 'bg-[#8ff5ff] border-[#8ff5ff] text-black' : 'bg-[#080808] border-white/5 text-slate-600'
                    }`}>
          ...
          <div className="space-y-6">
             <div className="bg-[#161616] p-6 rounded-[2rem] border border-white/5">
